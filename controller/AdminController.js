...
class AdminController {
  ...
  deleteAdmin = asyncTryCatch(async (req, res, next) => {
    ...
    const response = await adminService.deleteAdmin(req.params.adminId, token);
    const status = response.success ? 200 : 404;
    res.status(status).json(response);
  });

  // GET /api/v1/admin/allCustomers
  getAllCustomers = asyncTryCatch(async (req, res, next) => {
    const response = await adminService.getAllCustomers();
    console.log("Customers found:", response.data?.length || 0);
    res.status(200).json(response);
  });

  // GET /api/v1/admin/customer/:customerId (Fixed route to avoid conflict)
  getCustomerById = asyncTryCatch(async (req, res, next) => {
    const response = await adminService.getCustomerById(req.params.customerId);
    const status = response.success ? 200 : 404;
    res.status(status).json(response);
  });

  // DELETE /api/v1/admin/customer/:customerId (Fixed route to avoid conflict)
  deleteCustomer = asyncTryCatch(async (req, res, next) => {
    const response = await adminService.deleteCustomer(req.params.customerId);
    const status = response.success ? 200 : 404;
    res.status(status).json(response);
  });
}
